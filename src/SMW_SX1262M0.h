...
const char *const CMD_PNM = "PNM";           // Public Network Mode (3.4.7)
...
const char *const CMD_TX_TONE = "TXTONE";    // TX Tone Test (3.8.2)
...
const char *const CMD_LORA_TXH = "TXLRAH"; // TX LoRa P2P Hexa
const char *const CMD_LORA_RXH = "RXLRAH"; // RX LoRa P2P Hexa
...
const char *const RSPNS_LAST_COUNTER = "Last Counter=1 ";
const char RSPNS_LAST_COUNTER_HEX[] = {0x43, 0x6f, 0x75, 0x6e, 0x74, 0x65, 0x72, 0x3d, 0x31, 0x20, 0x0a, 0x0d};
const char RSPNS_RECVB_OK_HEX[] = {0x3A, 0x0D, 0x0A, 0x0D, 0x0A, 0x4F, 0x4B, 0x0D, 0x0A}; //:(D)(A)(D)(A)OK(D)(A)
// --------------------------------------------------
// Constants
...
public:
  ...
  CommandResponse tx_tone(uint32_t frequency);
  ...
  CommandResponse set_PNM(bool);
  char set_TCONF(uint32_t = 903000, uint8_t = 22, uint16_t = 500, uint8_t = 11, uint8_t = 5, bool = false, bool = false);
  CommandResponse P2P_startX(uint32_t, bool, const uint8_t * = nullptr, size_t = 0);

#ifdef SMW_SX1262M0_DEBUG
  void set_debugger(Stream *);
#endif
  void set_loop_cb(loop_cb);
