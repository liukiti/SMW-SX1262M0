...
SMW_SX1262M0::SMW_SX1262M0(Stream &stream) : _stream(&stream),
...
// --------------------------------------------------
// Start the TX Tone Test
//  @param (frequency) : the frequency to use for the wireless communication, in kHz [uint32_t]
//  @returns the type of the response [CommandResponse]
CommandResponse SMW_SX1262M0::tx_tone(uint32_t frequency)
{
  // check the frequency
  if ((frequency < 902000) || (frequency > 927800))
  {
  ...
  _send_command(CMD_TX_TONE, CommandAction::SET, 1, sfreq);
  return _read_response(SMW_SX1262M0_TIMEOUT_READ);
}
...
CommandResponse SMW_SX1262M0::readX(uint8_t(&port), Buffer(&buffer))
{
  ...
  // _stream_debug->printf("Parsing downlink msg: ");
  // for (int j = 0; j < _buffer.available(); j++)
  // {
  //   _stream_debug->printf("%0.2X", _buffer[j]);
  // }
  ...
  // // Edited. Basically initiates the search from the end.
  // uint8_t k = 0, h;
  // for (k = _buffer.available(); k >= 0; k--)
  // {
  //   if (_buffer[k] == CHAR_COLON)
  //   {
  //     break;
  //   }
  //   localBuffer.insert(_buffer[k]);
  // }

  // if (_buffer[k] == CHAR_COLON && k > 0)
  // {
  //   for (h = 0; (k > 0) && (h < 3); k--, h++)
  //   {
  //     if (!isdigit(_buffer[k - 1]))
  //     {
  //       break;
  //     }
  //   }
...
CommandResponse SMW_SX1262M0::set_AppEUI(const char *appeui)
{
  ...
  for (uint8_t i = 2; i < (length - 1); i += 3)
  { // ignore EOS
    // shift the data (start from the end)
    for (uint8_t j = (length - 2); j > i; j--)
    { // ignore EOS
      str[j] = str[j - 1];
    }
...
void SMW_SX1262M0::set_loop_cb(loop_cb func)
{
  lfunc = func;
}
// --------------------------------------------------
...
CommandResponse SMW_SX1262M0::set_PNM(bool value)
{
  uint8_t npm = (value) ? 1 : 0; // force binary value
  npm += '0';                    // convert to ASCII character, without a narrowing conversion
  unsigned char data[] = {npm, CHAR_EOS};
  // send the command and read the response
  _send_command(CMD_PNM, CommandAction::SET, 1, data);
  CommandResponse res = _read_response(SMW_SX1262M0_TIMEOUT_WRITE); // this command takes almost X s to reply

  return res;
}

char SMW_SX1262M0::set_TCONF(uint32_t frequency, uint8_t power, uint16_t bw, uint8_t sf, uint8_t crd, bool lna, bool pa)
{
  // check the frequency
  if ((frequency < 902000) || (frequency > 927800))
  {
  ...
  uint8_t slna = (lna) ? 1 : 0; // force binary value
  slna += '0';                  // convert to ASCII character, without a narrowing conversion
  unsigned char datalna[] = {slna, CHAR_EOS};

  uint8_t spa = (pa) ? 1 : 0; // force binary value
  spa += '0';                 // convert to ASCII character, without a narrowing conversion
  unsigned char datapa[] = {spa, CHAR_EOS};
  // send the command and read the response
  _send_command(CMD_LORA_CONFIG, CommandAction::SET, 7, sfreq, spower, sbw, ssf, scrd, datalna, datapa);
  CommandResponse res = _read_response(SMW_SX1262M0_TIMEOUT_WRITE); // this command takes almost X s to reply
  if (res == CommandResponse::OK)
    return 0;
  else
...
CommandResponse SMW_SX1262M0::P2P_startX(uint32_t frequency, bool continuous, const uint8_t *data, size_t len)
{
  // check the frequency
  if ((frequency < 902000) || (frequency > 927800))
  {
  ...
  if (data == nullptr)
  {
    _send_command(CMD_LORA_RXH, CommandAction::SET, 2, sfreq, mode);
  }
  else
  {
    // filter the data
    uint8_t length = 3 * len;
    char str[length];
    str[length - 1] = CHAR_EOS;

    for (int z = 0; z < len; z++)
    {
      if (((*(data + z) & 0xF0) >> 4) < 0x0A)
        str[2 * z] = ((*(data + z) & 0xF0) >> 4) + '0';
      else
        str[2 * z] = ((*(data + z) & 0xF0) >> 4) + '7';

      if ((*(data + z) & 0x0F) < 0x0A)
        str[(2 * z) + 1] = (*(data + z) & 0x0F) + '0';
      else
        str[(2 * z) + 1] = (*(data + z) & 0x0F) + '7';
    }
    ...
    _send_command(CMD_LORA_TXH, CommandAction::SET, 3, sfreq, mode, str);
  }

  return _read_response(SMW_SX1262M0_TIMEOUT_P2P);
}
...
CommandResponse SMW_SX1262M0::_read_response(uint32_t timeout)
{
  ...
  while (millis() < stop_time)
  {
    if (_stream->available())
    {
      ...
      length = _buffer.available();
      if (length >= sizeof(RSPNS_LAST_COUNTER_HEX))
      {
        uint8_t data[sizeof(RSPNS_LAST_COUNTER_HEX)];
        uint8_t sizeEqual = 0;
        for (int f = 0; f < sizeof(RSPNS_LAST_COUNTER_HEX); f++)
        {
          data[f] = _buffer[length - sizeof(RSPNS_LAST_COUNTER_HEX) + f];
          if (data[f] == RSPNS_LAST_COUNTER_HEX[f])
            sizeEqual++;
        }
        if (sizeEqual == sizeof(RSPNS_LAST_COUNTER_HEX))
          break;
      }
      if (length >= sizeof(RSPNS_RECVB_OK_HEX))
      {
        uint8_t data[sizeof(RSPNS_RECVB_OK_HEX)];
        uint8_t sizeEqual = 0;
        for (int f = 0; f < sizeof(RSPNS_RECVB_OK_HEX); f++)
        {
          data[f] = _buffer[length - sizeof(RSPNS_RECVB_OK_HEX) + f];
          if (data[f] == RSPNS_RECVB_OK_HEX[f])
          {
            sizeEqual++;
          }
        }
        if (sizeEqual == sizeof(RSPNS_RECVB_OK_HEX))
        {
          _stream_debug->printf(" (EXIT) ");
          break;
        }
      }
    }
  ...
  {
    _stream_debug->printf("\r\n");
    // _stream_debug->printf("length: %d | Data:", length);
    // for (int k = 0; k < length; k++)
    // {
    //   _stream_debug->printf("%0.2X", _buffer[k]);
    // }
    // _stream_debug->printf("\r\n");
  }
#endif
  ...
  if (buffer_length > 4)
  { // (the status is returned as "<CR><LF>Status<CR><LF>")
    uint8_t status = 0;
  ...
  if (_stream_debug)
  {
    // _stream_debug->printf("data2compare: ");
    // for (int j = 0; j < data_length; j++)
    //   _stream_debug->printf("%.02X", data[j]);
    _stream_debug->printf("\r\n");
  }
#endif

  if (memcmp(data, RSPNS_LAST_COUNTER, strlen(RSPNS_LAST_COUNTER)) == 0)
  {
#ifdef SMW_SX1262M0_DEBUG
    // debug
    if (_stream_debug)
    {
      //_stream_debug->printf("economy=%u\r\n", stop_time - realStop);
    }
#endif
